...
<head>
    ...
    <title>Editar Registro - Sistema Control</title>
    ...
    <main class="max-w-5xl mx-auto px-4 py-8 w-full flex-grow">
        <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-800">Modificar Registro Climático</h2>
                <p class="text-xs text-gray-500 mt-1">ID del registro: #{{ $monitoreo->id }}</p>
            </div>

            <form action="{{ route('monitoreo.update', $monitoreo->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="bg-stone-50 p-4 rounded-xl border border-stone-200 space-y-3">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" id="temperatura" name="temperatura" value="{{ $monitoreo->temperatura }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-0.5">Humedad (%)</label>
                            <input type="number" step="0.01" id="humedad" name="humedad" value="{{ $monitoreo->humedad }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-0.5">DPV (Auto)</label>
                            <input type="text" id="dpv_view" value="{{ $monitoreo->dpv }}" disabled class="w-full bg-gray-200 border border-gray-300 text-gray-600 rounded-lg px-2.5 py-1.5 text-sm font-mono font-bold">
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-200 space-y-3">
                        ...
                        <div>
                            ...
                            <input type="number" id="vol_riego_entrada" name="vol_riego_entrada" value="{{ $monitoreo->vol_riego_entrada }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-0.5">Vol. Drenaje Salida (mL)</label>
                            <input type="number" id="vol_drenaje_salida" name="vol_drenaje_salida" value="{{ $monitoreo->vol_drenaje_salida }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-blue-600 mb-0.5">% Drenaje (Auto)</label>
                            <input type="text" id="porcentaje_drenaje_view" value="{{ $monitoreo->porcentaje_drenaje }}%" disabled class="w-full bg-blue-100/80 border border-blue-200 text-blue-700 rounded-lg px-2.5 py-1.5 text-sm font-bold">
                        </div>
                    </div>
                    ...
                    <div class="bg-purple-50/50 p-4 rounded-xl border border-purple-200 space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input type="number" step="0.1" id="ph_entrada" name="ph_entrada" value="{{ $monitoreo->ph_entrada }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2 py-1 text-sm">
                            </div>
                            <div>
                                <label class="block text-[11px] font-medium text-gray-600 mb-0.5">pH Sal</label>
                                <input type="number" step="0.1" id="ph_salida" name="ph_salida" value="{{ $monitoreo->ph_salida }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2 py-1 text-sm">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-2 text-xs font-bold text-gray-500 pt-1">
                            <div>
                                <label class="mb-0.5 block">Dif. CE</label>
                                <input type="text" id="diferencia_ce_view" value="{{ $monitoreo->diferencia_ce }}" disabled class="w-full bg-gray-200 border rounded p-1 text-center font-mono">
                            </div>
                            <div>
                                <label class="mb-0.5 block">Dif. pH</label>
                                <input type="text" id="diferencia_ph_view" value="{{ $monitoreo->diferencia_ph }}" disabled class="w-full bg-gray-200 border rounded p-1 text-center font-mono">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-amber-50/30 p-4 rounded-xl border border-amber-200 space-y-4 md:col-span-2 flex flex-col justify-between">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                <input type="text" id="radiacion_semaforo_view" value="{{ $monitoreo->radiacion_semaforo }}" disabled class="w-full bg-gray-200 border border-gray-300 text-gray-600 rounded-lg px-2.5 py-2 text-sm font-bold text-center">
                                <input type="hidden" id="radiacion_semaforo" name="radiacion_semaforo" value="{{ $monitoreo->radiacion_semaforo }}">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Acción Tomada</label>
                                <textarea name="radiacion_accion_tomada" rows="1" placeholder="Describa la acción correctiva..." class="w-full bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-emerald-500 resize-none">{{ $monitoreo->radiacion_accion_tomada }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-amber-50/40 p-4 rounded-xl border border-amber-200 space-y-6 flex flex-col justify-between">
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                                <div>
                                    ...
                                    <input type="number" step="0.01" id="peso_tarde_anterior" name="peso_tarde_anterior" value="{{ $monitoreo->peso_tarde_anterior }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-2 text-sm focus:outline-emerald-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-600 mb-1">Peso Mañana (kg)</label>
                                    <input type="number" step="0.01" id="peso_manana" name="peso_manana" value="{{ $monitoreo->peso_manana }}" required class="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-2 text-sm focus:outline-emerald-500">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            ...
                            <input type="text" id="porcentaje_caida_nocturna_view" value="{{ $monitoreo->porcentaje_caida_nocturna }}%" disabled class="w-full bg-amber-100 border border-amber-200 text-amber-800 rounded-lg px-2.5 py-2 text-sm font-black text-center tracking-wider shadow-inner">
                        </div>
                    </div>
                ...
                <div class="w-full">
                    ...
                    @endphp
                    <div id="estatus_box" class="{{ $isOptimo ? 'bg-emerald-100 border-emerald-300' : 'bg-red-100 border-red-300' }} rounded-xl border p-6 flex flex-col justify-center items-center transition duration-300">
                        <span class="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Estatus Predictivo General</span>
                        <div id="estatus_text" class="text-2xl font-black {{ $isOptimo ? 'text-emerald-800' : 'text-red-800' }}">{{ $monitoreo->estatus_general }}</div>
                        <p class="text-[11px] text-gray-400 mt-2 text-center">Depende del rango óptimo del DPV (0.8 a 1.4)</p>
                    </div>
                </div>

                <div class="flex justify-end pt-4">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-lg shadow-md transition cursor-pointer">
                        <i class="fa-solid fa-arrow-rotate-right mr-2"></i> Actualizar Registro
                    </button>
                </div>
            </form>
        </div>
