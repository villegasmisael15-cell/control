...
<body>
    ...
    @if($bitacora->agroquimicos->isNotEmpty())
        ...
        @foreach($agroquimicosAgrupados as $grupoKey => $productosGrupo)
            ...
            <div style="margin-top: 10px; margin-bottom: 4px; background-color: #f9fafb; border: 1px solid #e5e7eb; padding: 6px 8px; font-size: 10px;">
                <strong>Fecha Aplicación:</strong> {{ \Carbon\Carbon::parse($primerDelGrupo->fecha_aplicacion)->format('d/m/Y') }} 
                &nbsp;&nbsp;|&nbsp;&nbsp; 
                <strong>Tipo:</strong> {{ $primerDelGrupo->aplicacion }}
            </div>

            <table class="data-table" style="margin-top: 2px;">
                <thead>
                    <tr>
                        <th style="width: 35%;">Producto / Ingrediente</th>
                        <th style="width: 25%;">Dosis</th>
                        <th style="width: 30%;">Observaciones</th>
                        <th style="width: 10%; text-align: center;">IS</th>
                    </tr>
                </thead>
                <tbody>
    ...
    @if($bitacora->fertilizantes->isNotEmpty())
        ...
        @if(!empty($tipoSolucionGeneral))
            <div style="margin-top: 8px; margin-bottom: 8px; background-color: #ecfdf5; border: 1px solid #a7f3d0; padding: 6px 10px; font-size: 10px; color: #065f46; font-weight: bold;">
                Tipo de Solución General: {{ $tipoSolucionGeneral }}
            </div>
        @endif

        @foreach($bitacora->fertilizantes->groupBy('tanque') as $nombreTanque => $accionesTanque)
            <div style="margin-top: 10px; margin-bottom: 4px; border-bottom: 1px solid #e5e7eb; padding-bottom: 3px;">
                <strong style="font-size: 11px; color: #059669; text-transform: uppercase;">Tanque: {{ $nombreTanque }}</strong>
            </div>
