        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
                ...
                <div class="bg-gray-50 px-5 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        @if(auth()->user()->rol === 'administrador')
                        <a href="{{ route('sanidad.edit', $bitacora->id) }}" class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold px-3 py-1.5 rounded-md transition shadow flex items-center gap-1">
                ...
                <div class="p-6 space-y-8 divide-y divide-gray-100">
                    ...
                    <!-- 1. MANEJO DE AGROQUÍMICOS (AGRUPADO POR FECHA Y TIPO DE APLICACIÓN) -->
                    <div class="space-y-4">
                        ...
                        <!-- BLOQUES MÚLTIPLES DE APLICACIONES -->
                        <div class="space-y-4 mt-4">
                            @foreach($agroquimicosAgrupados as $grupoKey => $productosGrupo)
                            @php $primerDelGrupo = $productosGrupo->first(); @endphp
                            <div class="bg-stone-50 border border-stone-200 rounded-xl p-4 shadow-2xs space-y-3">

                                <!-- ENCABEZADO DE LA APLICACIÓN (Fecha y Tipo únicos) -->
                                <div class="flex flex-wrap items-center justify-between gap-2 bg-white px-3 py-2 rounded-lg border border-stone-100 text-xs">
                                    <div class="flex items-center gap-3">
                                        <span class="font-bold text-gray-700 flex items-center gap-1">
                                            <i class="fa-solid fa-calendar text-gray-400"></i>
                                            {{ \Carbon\Carbon::parse($primerDelGrupo->fecha_aplicacion)->format('d/m/Y') }}
                                        </span>
                                        <span class="bg-orange-100 text-orange-800 text-[11px] px-2.5 py-0.5 rounded-md font-bold border border-orange-200">
                                            {{ $primerDelGrupo->aplicacion }}
                                        </span>
                                    </div>
                                </div>

                                <!-- TABLA DE PRODUCTOS DE ESTE GRUPO -->
                                <div class="overflow-x-auto rounded-lg border border-stone-200 bg-white">
                                    <table class="w-full text-left text-xs text-gray-600 border-collapse min-w-[650px]">
                                        <thead>
                                            <tr class="bg-stone-100 font-semibold text-stone-700 border-b border-stone-200">
                                                <th class="p-2 w-2/5">Producto / Ingrediente</th>
                                                <th class="p-2 w-1/4">Dosis / Unidad</th>
                                                <th class="p-2 w-1/3">Observaciones</th>
                                                <th class="p-2 text-center w-16">IS</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-stone-100">
                                            @foreach($productosGrupo as $arq)
                                            <tr class="hover:bg-stone-50/50">
                                                <td class="p-2 font-bold text-gray-900">
                                                    {{ $arq->producto }}
                                                </td>
                                                <td class="p-2 font-mono font-bold text-orange-700">
                                                    {{ $arq->dosis }} {{ $arq->unidad_dosis }}
                                                </td>
                                                <td class="p-2 text-gray-500 italic max-w-xs truncate" title="{{ $arq->observaciones }}">
                                                    {{ $arq->observaciones ?? '—' }}
                                                </td>
                    ...
                    <!-- 2. MANEJO DE FERTILIZANTES (CON TIPO DE SOLUCIÓN POR TANQUE) -->
                    <div class="space-y-4 pt-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-gray-200 pb-2">
                            ...
                            <!-- TIPO DE SOLUCIÓN GENERAL (AFUERA DE LOS TANQUES) -->
                            ...
                            @endphp
                            @if(!empty($tipoSolucionGeneral))
                            <div class="bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-lg text-xs font-bold text-emerald-800 flex items-center gap-1.5 self-start sm:self-auto">
                                <i class="fa-solid fa-circle-info text-emerald-600"></i>
                                <span>Tipo de Solución General: {{ $tipoSolucionGeneral }}</span>
                            </div>
                            @endif
                            @endif
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="bg-stone-50 border border-stone-200 rounded-lg p-4 shadow-2xs space-y-2">
                                ...
                                <div class="overflow-hidden rounded-lg border border-stone-200 bg-white mt-2">
