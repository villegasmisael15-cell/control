    ...
    public function exportarExcel($id)
{
    ...
    $nombreArchivo = "Reporte_Sector_" . str_replace(' ', '_', $monitoreo->sector) . "_ID_" . $monitoreo->id . ".csv";

    // Configurar cabeceras nativas para descarga de archivo CSV con soporte UTF-8 completo
    $headers = [
    ...
    $callback = function() use ($monitoreo, $caracteristicas) {
        $file = fopen('php://output', 'w');
        
        // Inyectar el BOM UTF-8 para que Excel reconozca e imprima correctamente acentos, eñes y símbolos (°C, m²)
        fprintf($file, chr(0xEF).chr(0xBB).chr(0xBF));

        // 1. CABECERA GENERAL (Sin el texto "ID de auditoría")
        fputcsv($file, ['BITÁCORA DE CONTROL HIDORPONÍA']);
        fputcsv($file, []); // Línea en blanco
        fputcsv($file, ['Fecha de Captura:', \Carbon\Carbon::parse($monitoreo->fecha)->format('d/m/Y')]);
        fputcsv($file, ['Sector:', $monitoreo->sector, 'Operador Evaluador:', $monitoreo->user ? $monitoreo->user->name : 'Sistema / Automatizado']);
        fputcsv($file, []);

        // 2. CARACTERÍSTICAS INICIALES DEL SECTOR
        fputcsv($file, ['1. CARACTERÍSTICAS INICIALES DEL ÁREA']);
        fputcsv($file, ['Superficie:', ($caracteristicas ? $caracteristicas->superficie_m2 : 'Sin datos') . ' m2', 'Variedad Instalada:', $caracteristicas ? $caracteristicas->variedad : 'Sin datos']);
        fputcsv($file, ['Fecha Trasplante:', $caracteristicas ? \Carbon\Carbon::parse($caracteristicas->fecha_trasplante)->format('d/m/Y') : 'Sin datos']);
        fputcsv($file, []);

        // 3. VARIABLES DIARIAS Y BALANCES
        fputcsv($file, ['2. VARIABLES MÉTRICAS Y BALANCES DIARIOS']);
        fputcsv($file, ['Temperatura Ambiente:', $monitoreo->temperatura . ' °C', 'Humedad Relativa:', $monitoreo->humedad . ' %']);
        fputcsv($file, ['DPV Calculado:', $monitoreo->dpv, 'Estatus General Clima:', $monitoreo->estatus_general]);
        fputcsv($file, ['Vol. Riego Entrada:', number_format($monitoreo->vol_riego_entrada) . ' mL', 'Vol. Drenaje Salida:', number_format($monitoreo->vol_drenaje_salida) . ' mL']);
        fputcsv($file, ['Porcentaje Drenaje:', $monitoreo->porcentaje_drenaje . '%', 'Caída Nocturna de Sustrato:', $monitoreo->porcentaje_caida_nocturna . '%']);
        fputcsv($file, []);

        // 4. CONDUCTIVIDAD Y PH
        fputcsv($file, ['3. PARÁMETROS QUÍMICOS Y DIFERENCIALES']);
        fputcsv($file, ['CE Entrada:', $monitoreo->ce_entrada, 'CE Salida:', $monitoreo->ce_salida, 'Diferencia CE:', $monitoreo->diferencia_ce]);
        fputcsv($file, ['pH Entrada:', $monitoreo->ph_entrada, 'pH Salida:', $monitoreo->ph_salida, 'Diferencia pH:', $monitoreo->diferencia_ph]);
        fputcsv($file, []);

        // 5. RADIACIÓN SOLAR
        fputcsv($file, ['4. RADIACIÓN SOLAR Y COMPORTAMIENTO']);
        fputcsv($file, ['Hora Captura Sol:', \Carbon\Carbon::parse($monitoreo->radiacion_hora)->format('g:i a'), 'Lectura Tomada:', number_format($monitoreo->radiacion_lectura) . ' Lux']);
        fputcsv($file, ['Semáforo Radiación:', $monitoreo->radiacion_semaforo, 'Acción Ejecutada:', $monitoreo->radiacion_accion_tomada ?? 'Ninguna']);
